...
export const fetchAllNotifications = async (): Promise<NotificationResponseDto[]> => {
...
export const fetchNotificationsByDataType = async (
  ...
  if (!response.ok) throw new Error(`Failed to fetch notifications for ${dataType}`);
  return response.json();
};
...
export const fetchNotificationByDataTypeAndDate = async (
  ...
  if (!response.ok) throw new Error(`Failed to fetch notification for ${dataType} at ${happenedAt}`);
  return response.json();
};
